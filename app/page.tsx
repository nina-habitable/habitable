...
export default function Home() {
  ...
  // ─── Filtered violations based on timeframe ───
  ...
  // ─── Summary based on timeframe ───
  const summary = useMemo(() => {
    if (!propertyData) return null;
    if (timeframe === "all") {
      // All-time: pass all violations, let summary treat them all as "recent"
      return generatePropertySummary(
        propertyData.violations.map((v) => ({
          class: v.class,
          novdescription: v.novdescription ?? "",
          // Pass a recent date so splitByRecency treats them all as recent
  ...
  // Hero search (no profile loaded)
  if (!hasProfile && !loading && !loadingProperty) {
    return (
                ...
                {topCategories.length > 0 && (
                  <p className="text-xs text-[var(--muted)] leading-relaxed mt-2">
                    ...
                    {topCategories
                      .map(
                        (c) =>
                          `${c.count} ${c.title.toLowerCase()}${c.count === 1 ? "" : ""}`
                      )
                      .join(", ")}
                    .
                  </p>
              ...
              return (
                <div className="rounded-xl border border-[var(--card-border)] bg-[var(--card)] p-5">
                  ...
                  {hasRecentInfestation ? (
                    ...
                    <div className="rounded-lg border border-[var(--card-border)] bg-[var(--background)] px-3 py-2 mb-3">
                      <p className="text-sm text-[var(--muted)]">
                        ...
                        {totalEradicated > 0 && ` ${totalEradicated} unit${totalEradicated === 1 ? "" : "s"} previously treated.`}
                      </p>
                    </div>
                  ) : null}
                  <div className="grid grid-cols-3 gap-3">
                    ...
                    <div>
                      <p className="text-lg font-bold text-[var(--foreground)]">
                        {totalEradicated > 0 ? totalEradicated : totalInfested}
                      </p>
                      <p className="text-[10px] text-[var(--muted-dim)]">
                        {totalEradicated > 0 ? "Units treated" : "Infested units reported"}
                      </p>
                    </div>
                  </div>
                </div>
